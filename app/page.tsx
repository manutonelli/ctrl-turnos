...
export default function Home() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/turnos?${new URLSearchParams({ action: "slots", from: days[0].date, to: days[days.length - 1].date })}`).then(r => r.json()).then(data => { if (!data.ok) throw new Error(data.error); setAvailable(new Set(data.slots.map((slot: { date: string; time: string }) => `${slot.date}|${slot.time}`))); }).catch(reason => setError(reason.message || "No pudimos cargar la agenda")).finally(() => setLoading(false));
  }, []);
  ...
  return <main className="min-h-screen bg-[var(--page-bg)] text-[var(--text)]"><div className="mx-auto max-w-[900px] px-5 pb-16 pt-6 sm:px-6 sm:pb-20 sm:pt-8"><BrandHeader onHome={goHome} onManage={() => { setManage(true); window.history.replaceState({}, "", "/?administrar=1"); }} />
    {step === "slots" && <section className="pt-12 sm:pt-16"><h1 className="text-[30px] font-medium leading-[1.14] tracking-[-.025em] sm:text-[40px]">Reservá tu consulta</h1><p className="mt-4 max-w-2xl text-[16px] leading-7 text-[var(--text-muted)] sm:text-[17px]">Consultas de 30 minutos en Av. M. Cabral 3009. La confirmación es inmediata.</p><div className="mt-10 rounded-[26px] bg-[var(--surface-2)] p-5 sm:mt-11 sm:rounded-[28px] sm:bg-white sm:p-8 sm:shadow-[0_18px_50px_rgba(16,47,85,.07)]"><p className="text-[11px] font-medium tracking-[.14em] text-[var(--text-faint)]">DÍA</p><div className="mt-3 flex gap-2 sm:gap-2.5">{days.map(item => <button key={item.date} onClick={() => { setDay(item.date); setTime(""); }} className={`min-w-0 flex-1 rounded-2xl border px-1 py-3 transition ${day === item.date ? "border-[var(--brand)] bg-[var(--brand)] text-white" : "border-[var(--border)] bg-white text-[var(--text-2)]"}`}><span className="block text-[11px] font-medium opacity-70 sm:text-xs">{item.short}</span><span className="mt-1 block text-[19px] font-medium sm:text-[22px]">{item.number}</span></button>)}</div><div className="mt-7 border-t border-[var(--divider)] pt-6"><p className="text-[11px] font-medium tracking-[.14em] text-[var(--text-faint)]">HORARIO</p><div className="mt-3 grid grid-cols-3 gap-2 sm:flex sm:flex-wrap">{times.map(slot => <button key={slot} onClick={() => setTime(slot)} className={`rounded-full border px-3 py-3 text-[14px] font-medium transition sm:px-5 sm:text-[15px] ${time === slot ? "border-[var(--brand-accent)] bg-[var(--selected-bg)] text-[var(--brand)]" : "border-[var(--border)] bg-white text-[var(--text-2)]"}`}>{slot}</button>)}</div>{loading && <p className="mt-4 text-sm text-[var(--text-muted)]">Cargando horarios…</p>}{!loading && !times.length && <p className="mt-4 text-[15px] text-[var(--text-muted)]">No hay horarios disponibles para este día.</p>}{error && <p className="mt-4 rounded-2xl bg-red-50 p-3 text-sm text-red-700">{error}</p>}</div><div className="mt-7 flex flex-wrap items-center justify-between gap-4 border-t border-[var(--divider)] pt-6"><p className="text-sm text-[var(--text-muted)]">{time ? `${selectedDay.label} a las ${time}` : "Elegí un día y un horario"}</p><Button disabled={!time} onClick={() => setStep("data")} className="h-12 w-full rounded-full bg-[var(--brand)] px-8 text-base hover:bg-[var(--brand-hover)] sm:w-auto">Continuar{time ? ` · ${selectedDay.short} ${selectedDay.number} ${time}` : ""}</Button></div></div><p className="mt-5 text-center text-xs text-[var(--text-faint)]">Te avisamos por WhatsApp el día anterior</p></section>}
    {step === "data" && <section className="pt-12 sm:pt-16"><button onClick={() => setStep("slots")} className="grid h-11 w-11 place-items-center rounded-full border border-[var(--border)] bg-white"><ArrowLeft size={18} /></button><h1 className="mt-8 text-[30px] font-medium tracking-[-.025em] sm:text-[36px]">Completá tus datos</h1><p className="mt-3 text-[var(--text-muted)]">Consulta presencial el {selectedDay.label.toLowerCase()} a las {time}.</p><form onSubmit={submit} className="mt-8 space-y-5 rounded-[28px] bg-white p-6 shadow-[0_18px_50px_rgba(16,47,85,.07)] sm:p-9">{[["name","Nombre y apellido","Ej: Manuela Gómez","text"],["whatsapp","WhatsApp","Ej: 2345 43-8544","tel"],["email","Email","nombre@email.com","email"],["notes","Observaciones","Contanos brevemente el motivo","text"]].map(([name,label,placeholder,type]) => <label key={name} className="block text-sm font-medium text-[var(--text-muted)]">{label}<Input name={name} required={name !== "notes"} type={type} placeholder={placeholder} className="mt-2 h-13 rounded-2xl border-[var(--border)] px-4 text-base" /></label>)}{error && <p className="rounded-2xl bg-red-50 p-3 text-sm text-red-700">{error}</p>}<Button className="h-13 w-full rounded-full bg-[var(--brand)] text-base hover:bg-[var(--brand-hover)]">Confirmar consulta</Button></form></section>}
