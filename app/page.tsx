...
import { ArrowLeft, Check, ChevronLeft, ChevronRight, LoaderCircle } from "lucide-react";
...
function businessDays(weekOffset = 0) {
  ...
  const cursor = new Date(); cursor.setHours(12, 0, 0, 0); cursor.setDate(cursor.getDate() + weekOffset * 7);
...
export default function Home() {
  ...
  const [weekOffset, setWeekOffset] = useState(0); const days = useMemo(() => businessDays(weekOffset), [weekOffset]);
  const [day, setDay] = useState(initialDays[0].date); const [time, setTime] = useState("");
  ...
  useEffect(() => {
    setLoading(true); setError(""); setTime("");
