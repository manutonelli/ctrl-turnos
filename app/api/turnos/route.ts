...
async function proxy(payload: Record<string, unknown>) {
  ...
  if (!runtime.GOOGLE_APPS_SCRIPT_URL || !runtime.CTRL_TURNOS_API_SECRET) {
    return Response.json(
      { ok: false, error: "La agenda todavía no está conectada" },
      { status: 503 },
    );
  }
  try {
    const response = await fetch(runtime.GOOGLE_APPS_SCRIPT_URL, {
      ...
      body: JSON.stringify({
        ...payload,
        secret: runtime.CTRL_TURNOS_API_SECRET,
      }),
      redirect: "follow",
    });
    ...
    if (!text.trim()) {
      return Response.json(
        {
          ok: false,
          error: `Google Apps Script respondió vacío (status ${response.status}). Revisá la implementación activa.`,
        },
        { status: 502 },
      );
    }
    try {
      ...
      return Response.json(
        {
          ok: false,
          error: `Google Apps Script devolvió una respuesta inválida (status ${response.status}). Verificá que la URL termine en /exec y que la implementación permita acceso a cualquier persona.`,
        },
        { status: 502 },
      );
    }
    ...
    return Response.json(
      {
        ok: false,
        error:
          "No se pudo conectar con Google Apps Script. Revisá la URL configurada en Cloudflare.",
      },
      { status: 502 },
    );
  }
...
export async function POST(request: Request) {
  ...
  if (String(payload.action || "").startsWith("admin")) {
    ...
    if (
      ...
      payload.adminPassword !== runtime.ADMIN_PASSWORD
    ) {
      return Response.json(
